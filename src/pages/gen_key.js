    ...
    view() {
        return m('.modal.show[role=dialog][aria-labelledby=key-generation-modal]',
            m('.modal-dialog[role=document]', m('.modal-content', [
                m('.modal-header',
                    m('h4.modal-title#key-generation-modal', 'Creation of your encryption key')
                ),
                m('.modal-body', [
                    this.error_message ? m('.alert .alert-danger', this.error_message) : '',

                    "Before starting, we must create your encryption key. In order to do that, you need to define a passphrase.",
                    m('br'),
                    "Your passphrase is known only by yourself. It's used to encrypt your data.", m('br'),
                    "It should contains at least 8 characters.",
                    "You can use a real phrase to memorize it more easily (ex: MyBajooAccount).",
                    m('br'), m('br'),
                    m('form#passphrase-input-form', { onsubmit: () => this.submit()}, [
                        ...
                        m('.checkbox', m('label', [
                            ...
                            "I don't want to use a passphrase"
                        ])),
                    ])
                ]),
                m('.modal-footer', [
                    m('button.btn.btn-primary[type=submit][form=passphrase-input-form]',
                        {disabled: this.is_loading}, 'Generate the key')
                ])
            ]))
        );
    }
    ...
    submit() {
        ...
        if (!this.no_passphrase) {
            ...
            if (this.passphrase().length < 8) {
                this.passphrase_error = 'The passphrase must have at least 8 characters';
                return;
            }
            if (this.passphrase() !== this.confirm()) {
                this.confirm_error = "The passphrase and its confirmation doesn't match";
                return;
            }
        }
        ...
        app.user.generate_key(app.user.email, passphrase).then(_ => m.route.set(app.redirect_to || '/'),
            err => {
                ...
                this.error_message = `Key generation failed: ${err.message || err}`;
                this.is_loading = false;
                m.redraw();
            });
    }
